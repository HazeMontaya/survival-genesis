...
class ToolRegistry:
    ...
    def call(self,tool_id,**kwargs):
        ...
        actor=str(kwargs.pop("_actor","genesis-1"))
        authority=str(kwargs.pop("_authority","self"))
        if self.policy:
            decision=self.policy.evaluate(tool_id,kwargs,actor=actor,authority=authority)
