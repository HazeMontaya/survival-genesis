...
class Treasury:
    ...
    def __init__(self, store):
        ...
        self.path = Path(os.getenv(
            "GENESIS_TREASURY_STATE",
            str(Path(store.path).parent / "treasury.json"),
        ))
    ...
    def _save(self):
        ...
        self.path.write_text(
            json.dumps(asdict(self.account), indent=2),
            encoding="utf-8",
        )
    ...
    def can_collect(self):
        return bool(
            self.policy.enabled
            and self.policy.collect_enabled
            and self.account
            and self.account.verified
        )

    def accept_signed_revenue(self, event_id, source, amount_eur, signature):
        secret = os.getenv("GENESIS_REVENUE_WEBHOOK_SECRET", "")
        if not secret:
            raise PermissionError("revenue webhook secret is not configured")
        if not event_id or event_id in (
            self.account.processed_event_ids if self.account else []
        ):
            raise ValueError("missing or replayed event")

        body = f"{event_id}|{source}|{amount_eur:.2f}".encode()
        expected = hmac.new(secret.encode(), body, hashlib.sha256).hexdigest()
        if not hmac.compare_digest(signature or "", expected):
            raise PermissionError("invalid revenue signature")
        if not self.can_collect():
            raise PermissionError("treasury collection is not enabled and verified")

        from .economy import RevenueEvent, Economy
        ledger = Economy(self.store).record_revenue(
            RevenueEvent(str(source), float(amount_eur), True)
        )
        ...
        self._save()
        self.store.event(
            "treasury_revenue_verified",
            {"event_id": event_id, "source": source, "amount_eur": amount_eur},
        )
        return ledger
    ...
    def payout_destination_allowed(self, destination):
        return bool(
            destination
            and self.policy.allowed_payout_hashes
            and self.fingerprint(
                self.account.provider if self.account else "",
                str(destination),
            ) in self.policy.allowed_payout_hashes
        )
    ...
    def can_payout(
        ...
        if self.policy.max_payout_eur <= 0 or amount_eur > self.policy.max_payout_eur:
            return False
        if (
            self.policy.daily_payout_limit_eur <= 0
            or today_payout_eur + amount_eur > self.policy.daily_payout_limit_eur
        ):
            return False
        if current_cash_eur - amount_eur < self.policy.minimum_cash_reserve_eur:
            return False
        if not self.payout_destination_allowed(destination):
            return False
    ...
    def can_trade(self, notional_eur=0.0, today_notional_eur=0.0, current_cash_eur=0.0):
        if not (
            self.policy.enabled
            and self.policy.trading_enabled
            and self.account
            and self.account.verified
            and self.policy.trading_mode == "live"
        ):
            return False
        if notional_eur <= 0 or self.policy.daily_trade_notional_eur <= 0:
            return False
        if today_notional_eur + notional_eur > self.policy.daily_trade_notional_eur:
            return False
